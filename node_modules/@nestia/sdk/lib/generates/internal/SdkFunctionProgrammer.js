...
(function (SdkFunctionProgrammer) {
    SdkFunctionProgrammer.generate = (config) => (importer) => (route) => {
        const [x, y, z] = [head, body, tail].map((closure) => closure(config)(importer)(route)({
            headers: route.parameters.find((param) => param.category === "headers" && param.field === undefined),
            query: route.parameters.find((param) => param.category === "query" && param.field === undefined),
        ...
        return (`export namespace ${route.name} {\n` +
            ...
            [
                ...
                ...(route.output.contentType === "application/x-www-form-urlencoded"
                    ? [
                        `    parseQuery: (input: URLSearchParams) => ${SdkImportWizard_1.SdkImportWizard.typia(importer)}.http.assertQuery<${route.output.typeName}>(input),`,
                    ]
                    : []),
                "} as const;",
            ]
