...
(function (E2eFileProgrammer) {
    E2eFileProgrammer.generate = (config) => (props) => (route) => __awaiter(this, void 0, void 0, function* () {
        ...
        const content = [importer.toScript(props.current), "", body].join("\n");
    ...
    const arrow = (config) => (importer) => (route) => {
        ...
        return [
            ...
            ...(route.output.typeName === "void"
                ? [`    ${output}`]
                : [
                    `    const output: ${SdkTypeDefiner_1.SdkTypeDefiner.output(config)(importer)(route)} = ${output}`,
    ...
    const wrap = (config) => (importer) => (name, body) => config.primitive === false
        ? name
        : `${(body ? SdkImportWizard_1.SdkImportWizard.Primitive : SdkImportWizard_1.SdkImportWizard.Resolved)(importer)}<${name}>`;
